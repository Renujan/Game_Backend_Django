...
def get_puzzle(request):
    ...
    # If no puzzles in DB, fetch from Banana API
    try:
        # This is a placeholder - replace with actual Banana API endpoint
        banana_api_url = "https://api.banana.com/puzzle"  # Replace with actual URL
        response = requests.get(banana_api_url)
        if response.status_code == 200:
            puzzle_data = response.json()
            # Map difficulty and set points/time based on level
            difficulty = puzzle_data.get('difficulty', 'medium')
            points_map = {'easy': 10, 'medium': 20, 'hard': 30}
            points_value = points_map.get(difficulty, 20)
    ...
    # Fallback: create a sample puzzle
    sample_puzzle = Puzzle.objects.create(
    ...
    serializer = PuzzleSerializer(sample_puzzle)
    return Response(serializer.data)
...
def check_answer(request):
    ...
    if not puzzle_id or not answer:
        return Response({'error': 'puzzle_id and answer are required'}, status=status.HTTP_400_BAD_REQUEST)

    try:
        puzzle = Puzzle.objects.get(puzzle_id=puzzle_id)
        correct_solution = puzzle.get_solution()

        is_correct = answer.strip().lower() == correct_solution.strip().lower()
        ...
        # Update player stats
        request.user.total_games_played = F('total_games_played') + 1
        if is_correct:
            request.user.total_correct_answers = F('total_correct_answers') + 1
            request.user.score = F('score') + points_earned
        request.user.save()
...
def leaderboard(request):
    ...
    # Calculate accuracy and get top players
...
def admin_players(request):
    players = Profile.objects.all().order_by('-date_joined')
